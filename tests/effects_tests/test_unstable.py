...
def test_unstable_effect(input_data, terminal_config_default) -> None:
...
@pytest.mark.parametrize("input_data", ["single_char", "medium"], indirect=True)
def test_unstable_explosion_ease(terminal_config_default, input_data, easing_function_1) -> None:
    effect = effect_unstable.Unstable(input_data)
    effect.terminal_config = terminal_config_default
...
@pytest.mark.parametrize("input_data", ["single_char", "medium"], indirect=True)
def test_unstable_reassembly_ease(terminal_config_default, input_data, easing_function_1) -> None:
    effect = effect_unstable.Unstable(input_data)
    effect.terminal_config = terminal_config_default
