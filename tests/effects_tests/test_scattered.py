"""Tests for the Scattered effect and its configuration surface."""
...
def test_scattered_effect(input_data: str, terminal_config_default_no_framerate: TerminalConfig) -> None:
    """Test the Scattered effect against a variety of representative inputs."""
...
def test_scattered_effect_terminal_color_options(
    input_data: str,
    terminal_config_with_color_options: TerminalConfig,
) -> None:
    """Test Scattered output when terminal color toggles change."""
    effect = effect_scattered.Scattered(input_data)
...
def test_scattered_final_gradient(
    ...
    input_data: str,
    gradient_direction: effect_scattered.Gradient.Direction,
    gradient_steps: tuple[int, ...],
    gradient_stops: tuple[Color, ...],
) -> None:
    """Verify the Scattered effect respects final gradient settings."""
    effect = effect_scattered.Scattered(input_data)
...
def test_scattered_args(
    terminal_config_default_no_framerate: TerminalConfig,
    input_data: str,
    movement_speed: float,
    easing_function_1: effect_scattered.easing.EasingFunction,
) -> None:
    """Ensure Scattered accepts and renders with various configuration arguments."""
    effect = effect_scattered.Scattered(input_data)
    effect.terminal_config = terminal_config_default_no_framerate
...
def test_scattered_dynamic_without_preexisting_colors_uses_no_color_throughout() -> None:
    """Verify dynamic mode keeps uncolored input uncolored for the entire synced scene."""
    effect = effect_scattered.Scattered("A")
    effect.terminal_config = _make_terminal_config("dynamic")

    iterator = cast("effect_scattered.ScatteredIterator", iter(effect))
...
def test_scattered_dynamic_with_preexisting_fg_uses_input_fg_throughout() -> None:
    """Verify dynamic mode uses the parsed foreground color for the entire synced scene."""
    effect = effect_scattered.Scattered("\x1b[38;5;196mA\x1b[0m")
    effect.terminal_config = _make_terminal_config("dynamic")
...
def test_scattered_dynamic_with_preexisting_bg_only_uses_input_bg_throughout() -> None:
    """Verify dynamic mode uses the parsed background color without inventing a foreground."""
    effect = effect_scattered.Scattered("\x1b[48;5;106mA\x1b[0m")
    effect.terminal_config = _make_terminal_config("dynamic")
...
def test_scattered_dynamic_with_preexisting_fg_and_bg_uses_input_colors_throughout() -> None:
    """Verify dynamic mode uses parsed foreground and background colors for the entire synced scene."""
    effect = effect_scattered.Scattered("\x1b[38;5;196m\x1b[48;5;106mA\x1b[0m")
    effect.terminal_config = _make_terminal_config("dynamic")
...
def test_scattered_ignore_with_preexisting_colors_uses_effect_gradient() -> None:
    """Verify ignore mode keeps the effect-owned synced gradient."""
    effect = effect_scattered.Scattered("\x1b[38;5;196mA\x1b[0m")
...
def test_scattered_always_with_preexisting_colors_uses_input_colors() -> None:
