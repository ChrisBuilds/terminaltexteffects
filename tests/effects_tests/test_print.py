"""Tests for the Print effect and its configuration surface."""
...
def test_print_effect(input_data: str, terminal_config_default_no_framerate: TerminalConfig) -> None:
    """Test the Print effect against a variety of representative inputs."""
...
def test_print_effect_terminal_color_options(
    input_data: str,
    terminal_config_with_color_options: TerminalConfig,
) -> None:
    """Test Print output when terminal color toggles change."""
    effect = effect_print.Print(input_data)
...
def test_print_final_gradient(
    ...
    input_data: str,
    gradient_direction: effect_print.Gradient.Direction,
    gradient_steps: tuple[int, ...],
    gradient_stops: tuple[Color, ...],
) -> None:
    """Verify the Print effect respects final gradient settings."""
    effect = effect_print.Print(input_data)
...
def test_print_args(
    ...
    print_speed: int,
    easing_function_1: effect_print.easing.EasingFunction,
) -> None:
    """Ensure Print accepts and renders with various configuration arguments."""
    effect = effect_print.Print(input_data)
    effect.terminal_config = terminal_config_default_no_framerate
...
def test_print_dynamic_without_preexisting_colors_has_uncolored_final_frame() -> None:
    """Verify dynamic mode leaves uncolored input uncolored at the end of typing."""
    effect = effect_print.Print("A")
...
def test_print_dynamic_with_preexisting_fg_uses_input_fg_color() -> None:
    """Verify dynamic mode restores a parsed foreground color at the end of typing."""
    effect = effect_print.Print("\x1b[38;5;196mA\x1b[0m")
    effect.terminal_config = _make_terminal_config("dynamic")
...
def test_print_dynamic_with_preexisting_bg_only_uses_input_bg_color() -> None:
    """Verify dynamic mode restores a parsed background color without inventing a foreground."""
    effect = effect_print.Print("\x1b[48;5;106mA\x1b[0m")
    effect.terminal_config = _make_terminal_config("dynamic")
...
def test_print_dynamic_with_preexisting_fg_and_bg_uses_input_colors() -> None:
    """Verify dynamic mode restores parsed foreground and background colors together."""
    effect = effect_print.Print("\x1b[38;5;196m\x1b[48;5;106mA\x1b[0m")
    effect.terminal_config = _make_terminal_config("dynamic")
...
def test_print_ignore_with_preexisting_colors_uses_effect_gradient() -> None:
...
def test_print_always_with_preexisting_colors_uses_input_colors() -> None:
