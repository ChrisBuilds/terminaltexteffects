...
def test_eventhandler_register_event_invalid_caller(event, eventhandler: EventHandler):
    with pytest.raises(ValueError):
        eventhandler.register_event(event, "invalid_caller", EventHandler.Action.ACTIVATE_PATH, "target")  # type: ignore
...
def test_eventhandler_register_event(eventhandler: EventHandler):
    ...
    eventhandler.registered_events[(EventHandler.Event.PATH_COMPLETE, p1)] == (EventHandler.Action.ACTIVATE_PATH, p2)
...
def test_eventhandler_handle_event(eventhandler: EventHandler):
    ...
    eventhandler.character.motion.active_path == p2
...
def test_effectcharacter_tick_no_paths_or_scenes(effectcharacter: EffectCharacter):
    effectcharacter.tick()


def test_effectcharacter_tick_scene_and_path(effectcharacter: EffectCharacter):
    p = effectcharacter.motion.new_path()
    ...
    effectcharacter.tick()
    assert effectcharacter.animation.active_scene.frames[0].ticks_elapsed == 1  # type: ignore
    assert effectcharacter.motion.active_path.current_step == 1  # type: ignore


def test_effectcharacter_equal_invalid_type(effectcharacter: EffectCharacter):
