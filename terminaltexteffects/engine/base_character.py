...
class EffectCharacter:
    ...
    @property
    def layer(self) -> int:
        """Rendering layer used to resolve character collisions."""
        return self._layer

    @layer.setter
    def layer(self, layer: int) -> None:
        """Set the rendering layer and invalidate terminal painter ordering when necessary."""
        ...
        owner = self._terminal_owner_token() if self._terminal_owner_token is not None else None
        if owner is not None:
            owner._notify_character_layer_changed(self, previous_layer)
