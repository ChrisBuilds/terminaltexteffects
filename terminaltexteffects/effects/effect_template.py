...
def get_effect_resources() -> tuple[str, type[BaseEffect], type[BaseConfig]]:
    ...
    """
    return "dev", Effect, EffectConfig
...
class EffectIterator(BaseEffectIterator[EffectConfig]):
    ...
    def __init__(self, effect: Effect) -> None:
        ...
        self.spawner = self.terminal.add_character("#", tte.Coord(1, 1))
        self.particle_gap = 10
        self.frames_until_particle = 0
        self.pool = self._setup_pool()
        self.build()

    def _setup_pool(self) -> ParticlePool:
        def _setup_particle(particle_char: tte.EffectCharacter) -> None:
            fade_scn = particle_char.animation.new_scene(scene_id="fade")
            for color in tte.Gradient(tte.Color("ff0000"), tte.Color("000000"), steps=10):
                fade_scn.add_frame("0", 4, colors=tte.ColorPair(fg=color))
        ...
        pool = ParticlePool(
            ...
            symbols="O",
            initial_count=1000,
            initializer=_setup_particle,
        )
        for particle in pool.particles:
            pool.reclaim_on_event(
                particle,
                caller=particle.animation.query_scene("fade"),
            )
        return pool

    def build(self) -> None:
        """Build the effect."""
        p = self.spawner.motion.new_path(path_id="route", speed=0.8, ease=tte.easing.out_bounce)
        for _ in range(10):
            p.new_waypoint(self.terminal.canvas.random_coord(), bezier_control=self.terminal.canvas.random_coord())
        self.spawner.motion.activate_path(p)
        self.terminal.set_character_visibility(self.spawner, is_visible=True)
        self.active_characters.add(self.spawner)

    def __next__(self) -> str:
        """Return the next frame of the effect."""
        if self.pending_chars or self.active_characters:
            if not self.frames_until_particle and self.spawner in self.active_characters:
                self.pool.emit(
                    origin=self.spawner.motion.current_coord,
                    on_emit=lambda c: c.animation.activate_scene("fade"),
                )
                self.frames_until_particle = self.particle_gap
            else:
                self.frames_until_particle -= 1
            self.update()
            return self.frame
        raise StopIteration
