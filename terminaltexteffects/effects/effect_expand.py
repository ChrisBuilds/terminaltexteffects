...
from terminaltexteffects import Color, EffectCharacter, EventHandler, Gradient, Scene, easing
...
class ExpandConfig(BaseConfig):
    ...
    parser_spec = argutils.ParserSpec(
        ...
        epilog=(
            ...
            "--final-gradient-direction vertical --movement-speed 0.35 --expand-easing IN_OUT_QUART"
        ),
    )
...
class ExpandIterator(BaseEffectIterator[ExpandConfig]):
    ...
    def build(self) -> None:
        ...
        for character in self.terminal.get_characters():
            ...
            if self.terminal.config.existing_color_handling == "dynamic":
                fg_gradient = (
                    Gradient(final_gradient.spectrum[0], character.animation.input_fg_color, steps=10)
                    if character.animation.input_fg_color
                    else None
                )
                bg_gradient = (
                    Gradient(final_gradient.spectrum[0], character.animation.input_bg_color, steps=10)
